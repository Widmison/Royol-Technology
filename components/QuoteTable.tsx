...
export default function QuoteTable({ quotes }: { quotes: any[] }) {
  ...
  const quoteActions = (quote: any, layout: "card" | "table") => {
    if (quote.status === "PENDING_DROPOFF") {
      return (
        <div
          className={
            layout === "card"
              ? "flex flex-col gap-2 sm:flex-row sm:flex-wrap"
              : "flex flex-col items-end gap-2 sm:flex-row sm:justify-end"
          }
        >
          <button
            type="button"
            ...
            className="bg-mex-dark text-white text-xs font-bold px-4 py-2.5 rounded-lg shadow hover:bg-gray-800 transition-colors w-full sm:w-auto"
          >
            Weigh & Invoice
          </button>
          <button
            ...
            className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-bold text-red-700 transition hover:bg-red-100 disabled:opacity-50 w-full sm:w-auto"
            title="Delete quote"
          >
    ...
    if (layout === "table") {
      return <div className="flex justify-end">{pipeline}</div>;
    }
    return pipeline;
  };
  ...
  return (
    <>
      <div className="lg:hidden space-y-3">
        {quotes.length === 0 ? (
          <div className="rounded-2xl border border-gray-100 bg-white p-6 text-center text-gray-500 font-medium shadow-sm">
          ...
          quotes.map((quote: any) => (
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                ...
                <span
                  className={`shrink-0 px-3 py-1 rounded-full text-[10px] uppercase font-bold tracking-wider ${statusBadge(quote.status)}`}
                >
              ...
              <div className="pt-1 border-t border-gray-50">{quoteActions(quote, "card")}</div>
            </div>
          ))
        )}
      </div>

      <div className="hidden lg:block overflow-x-auto overscroll-x-contain -mx-px">
            ...
            {quotes.length === 0 ? (
              ...
              quotes.map((quote: any) => (
                <tr key={quote.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-4 text-right">{quoteActions(quote, "table")}</td>
                </tr>
              ))
            )}
          </tbody>
